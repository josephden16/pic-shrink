...
Usage: pic-shrink <image-file-path>`).option("i",{alias:"imagesPath",describe:"Path to the directory containing the images you want to compress.",type:"string",demandOption:!1}).option("o",{alias:"outputPath",describe:"Path to the directory where you want to store the compressed images",type:"string",demandOption:!1}).option("c",{alias:"compressionLevel",describe:"Optional value between 1 and 10 to specify the compression level for the images",type:"number",demandOption:!1}).example([["pic-shrink image.jpg","Compress a single image file"],["pic-shrink image.jpg -c 4","Compress a single image file with a custom compression level"],["pic-shrink -i ./images","Compress images in the `images` directory and store the compressed images in the same directory"],["pic-shrink -i ./images -o ./compressed","Compress images in the `images` directory and store them in the `compressed` folder"],["pic-shrink -i ./images -c 8","Compress images in the `images` directory using a custom compression level of `8`"]]).parse()}var r=require("path"),M=f(require("sharp"));var g=require("fs");async function L(e,i,t){let n=(0,M.default)(e),a=(await n.metadata()).format,p=h(e),s={jpeg:{quality:70},webp:{quality:70},png:{compressionLevel:7}},m;i&&(m=parseInt(i),m<1&&(m=1),m>10&&(m=10),s.jpeg.quality=m*10,s.webp.quality=m*10,s.png.compressionLevel=m);let{dir:P,base:w}=(0,r.parse)(e);if(t){let c=(0,r.join)(t,w);await n[a](s[a]).toFile(c);let I=h(c);x(w,I,p)}else{let c=C(e),I=`${P}/${c}`;await n[a](s[a]).toFile(`${P}/${c}`);let R=h(I);x(w,R,p)}}async function N(e,i){let t=(0,r.resolve)(e);if(S(t)){F(t,"",!0),await L(t,i,""),z(C(t));return}else throw new Error("Error: Invalid image passed")}async function B(e,i,t){if(!e)throw new Error("Error: Images path not specified");let n=(0,r.resolve)(e);if(!(0,g.existsSync)(n))throw new Error("Error: Images file path is invalid");let o;i&&(o=(0,r.resolve)(i),o&&!(0,g.existsSync)(o)&&(0,g.mkdir)(o,{recursive:!0},s=>{if(s)throw new Error(`Error: Failed to create new directory at ${o}`)})),F((0,r.resolve)(e),i?(0,r.resolve)(i):""),await l(700),console.log("Scanning directory for images...");let a=(0,g.readdirSync)(n).map(s=>(0,r.join)(n,s)).filter(b).filter(S);if(a.length===0)throw new Error(`Error: No images found at: ${n}`);await l(1e3),console.log(`Found ${a.length} images.
`),k(),await l(1e3),console.log(`
Compressing images...
`),await l(1600);let p=0;Promise.all(a.map(async s=>{await L(s,t,o),p+=1})).then(async()=>{await l(1e3),O(p)})}var G=f(require("yargs"));async function K(){try{if(process.argv.length<3){G.default.showHelp();return}let{compressionLevel:e,imagesPath:i,outputPath:t}=E(),n=process.argv[2];n&&!i?await N(n,e):i&&await B(i,t,e)}catch(e){console.error(q.default.red(e.message))}}function U(){j(),K()}U();
