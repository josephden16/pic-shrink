...
Usage: pic-shrink <image-file-path>`).option("i",{alias:"imagesPath",describe:"Path to the directory containing the images you want to compress.",type:"string",demandOption:!1}).option("o",{alias:"outputPath",describe:"Path to the directory where you want to store the compressed images",type:"string",demandOption:!1}).option("c",{alias:"compressionLevel",describe:"Optional value between 1 and 10 to specify the compression level for the images",type:"number",demandOption:!1}).example([["pic-shrink image.jpg","Compress a single image file"],["pic-shrink image.jpg -c 4","Compress a single image file with a custom compression level"],["pic-shrink -i ./images","Compress images in the `images` directory and store the compressed images in the same directory"],["pic-shrink -i ./images -o ./compressed","Compress images in the `images` directory and store them in the `compressed` folder"],["pic-shrink -i ./images -c 8","Compress images in the `images` directory using a custom compression level of `8`"]]).parse()}var a=require("path"),M=f(require("sharp"));var l=require("fs");async function L(e,i,t){let n=(0,M.default)(e),s=(await n.metadata()).format,g=h(e),r={jpeg:{quality:70},webp:{quality:70},png:{compressionLevel:7}},m;i&&(m=parseInt(i),m<1&&(m=1),m>10&&(m=10),r.jpeg.quality=m*10,r.webp.quality=m*10,r.png.compressionLevel=m);let{dir:P,base:w}=(0,a.parse)(e);if(t){let c=(0,a.join)(t,w);await n[s](r[s]).toFile(c);let I=h(c);x(w,I,g)}else{let c=C(e),I=`${P}/${c}`;await n[s](r[s]).toFile(`${P}/${c}`);let R=h(I);x(w,R,g)}}async function N(e,i){let t=(0,a.resolve)(e);if(S(t)){F(t,"",!0),await p(1e3),await L(t,i,""),await p(1e3),z(C(t));return}else throw new Error("Error: Invalid image passed")}async function B(e,i,t){if(!e)throw new Error("Error: Images path not specified");let n=(0,a.resolve)(e);if(!(0,l.existsSync)(n))throw new Error("Error: Images file path is invalid");let o;i&&(o=(0,a.resolve)(i),o&&!(0,l.existsSync)(o)&&(0,l.mkdir)(o,{recursive:!0},r=>{if(r)throw new Error(`Error: Failed to create new directory at ${o}`)})),F((0,a.resolve)(e),i?(0,a.resolve)(i):""),await p(700),console.log("Scanning directory for images...");let s=(0,l.readdirSync)(n).map(r=>(0,a.join)(n,r)).filter(b).filter(S);if(s.length===0)throw new Error(`Error: No images found at: ${n}`);await p(1e3),console.log(`Found ${s.length} images.
`),k(),await p(1e3),console.log(`
Compressing images...
`),await p(1600);let g=0;Promise.all(s.map(async r=>{await L(r,t,o),g+=1})).then(async()=>{await p(1e3),O(g)})}var G=f(require("yargs"));async function K(){try{if(process.argv.length<3){let s=await G.default.getHelp();throw new Error(s)}let{compressionLevel:e,imagesPath:i,outputPath:t}=E(),n=process.argv[2];n&&!i?await N(n,e):i&&await B(i,t,e)}catch(e){console.error(q.default.red(e.message))}}function U(){j(),K()}U();
